...
[![CI Tests](https://img.shields.io/badge/Tests-67%20Passed-brightgreen?style=for-the-badge&logo=checkmarx)](test/)
...
*Zero External Dependencies · 100% Self-Contained Vibeguard Secret Defense · Flat-YAML Source of Truth · Mathematical Salience Ranking · Dual MCP & CLI*
...
1. `musememory` automatically detects or creates a `.memory/` folder in your project.
...
| :--- | :---: | :--- |
| **Atomic File Engine** | ✅ Complete | Atomic tmp-rename file storage with zero database locks. |
...
| **Auto Root & Workspace Init** | ✅ Complete | `musememory init` and auto `.memory/` bootstrapping in any workspace. |
...
| **Automated Test Suite** | ✅ Complete | 67 tests passing across 10 suites with clean TypeScript static checks. |
...
| :--- | :--- | :--- |
| `init` | `[path]` | Initialize `.memory/` directory and `CURRENT.md` in workspace. |
...
docker run -it -v $(pwd)/.memory:/app/.memory musememory
```
...
```bash
bun install
bun test          # 67 tests passing across 10 test suites
